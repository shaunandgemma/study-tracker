...
const migrationPath = path.join(root, 'supabase', 'migrations', '20260828_publish_terraform_004_exam_questions.sql');

test('Terraform 004 is configured to prefer Supabase while retaining 30 local questions', () => {
  assert.equal(TERRAFORM_ASSOCIATE_EXAM.questionSource, 'supabase');
  assert.equal(TERRAFORM_ASSOCIATE_EXAM.questions.length, 30);
  assert.equal(new Set(TERRAFORM_ASSOCIATE_EXAM.questions.map(question => question.id)).size, 30);
});

test('Terraform question migration is guarded and scoped away from AWS questions', () => {
  const sql = fs.readFileSync(migrationPath, 'utf8');

  for (const question of TERRAFORM_ASSOCIATE_EXAM.questions) {
    assert.match(sql, new RegExp(`\\$tfq\\$${question.id}\\$tfq\\$`));
  }

  assert.match(sql, /exam_code <> 'terraform-associate-004'/);
  assert.match(sql, /expected 30 seed questions/);
  assert.match(sql, /topic mapping count is not 30/);
...
test('question service calls accept the bundled bank as an offline fallback', () => {
  ...
  assert.match(setup, /activeExam\.questions \|\| null/);
});
