...
const folder = path.join(root, 'docs', 'author-assistant', 'handoffs', 'author-assistant-terraform-variables-validation-codex-20260815-001');
...
test('Terraform variables handoff is complete, valid and local-only', async () => {
  ...
  assert.equal(content.programme.programmeId, 'terraform-variables-validation-learning-path');
  ...
  assert.equal(handoff.summary.checkboxCount, 107);
  assert.equal(handoff.summary.cliCommandCount, 45);
  ...
  const preview = await validateAuthorHandoffImportPreview({ handoffPackage: handoff, acceptance, currentUser: { id: 'test-author', email: 'author@example.com' }, cryptoImpl: globalThis.crypto });
  assert.equal(preview.valid, true);
  assert.equal(preview.readOnly, true);
  assert.equal(preview.canSaveDraft, false);
});

test('Terraform Card 4 teaches complex values, expressions, precedence, safe failures and cleanup', () => {
  const handoff = JSON.parse(fs.readFileSync(packagePath, 'utf8'));
  ...
  for (const required of [
    ...
    'merge(local.effective_tags',
    'precondition',
    'at_least_one_parameter',
    'training.auto.tfvars',
    'expanded.tfvars',
    'invalid-environment.tfvars',
    'invalid-parameter.tfvars',
    'terraform plan -destroy -var-file=expanded.tfvars -out=destroy.tfplan'
