...
const correctedBankPath = path.join(projectRoot, 'data', 'saa-c03-question-export-corrected.json');
...
export function normalizeAndValidateQuestions(sourceQuestions) {
  ...
  const normalizedQuestions = sourceQuestions.map((question, index) => {
    ...
    if (
      ...
      validationErrors.push(`${label}: correctAnswer must be a valid zero-based option index.`);
    }

    if (!Array.isArray(question.correctAnswers)) {
      validationErrors.push(`${label}: correctAnswers must be an array.`);
    } else {
      const uniqueCorrectAnswers = new Set(question.correctAnswers);

      if (uniqueCorrectAnswers.size !== question.correctAnswers.length) {
        validationErrors.push(`${label}: correctAnswers must contain unique indexes.`);
      }
      ...
      if (
        ...
        validationErrors.push(`${label}: correctAnswers contains an invalid zero-based option index.`);
      }

      if (
        question.type === 'single' &&
        (question.correctAnswers.length !== 1 || question.correctAnswers[0] !== question.correctAnswer)
      ) {
        validationErrors.push(
          `${label}: Single-answer questions must have exactly one correctAnswers entry matching correctAnswer.`
        );
      }

      if (
        question.type === 'multiple' &&
        (![2, 3].includes(question.correctAnswers.length) ||
          !question.correctAnswers.includes(question.correctAnswer))
      ) {
        validationErrors.push(
          `${label}: Multiple-answer questions must have 2 or 3 unique correct indexes and include correctAnswer.`
        );
      }
    }
