...
const SOURCE_EC2_BATCH_PATH = 'E:/code/AWS Tool/migration_export/hands_on_tasks/batches/SAA/ec2.json';
const DUPLICATES_PATH = 'E:/code/AWS Tool/migration_export/hands_on_tasks/duplicate-groups.json';
...
function checkPrimaryTopic(task) {
  ...
  if (title.includes('vpc peering') || title.includes('transit gateway') || title.includes('vpn')) {
    return 'topic-vpc';
  }
  if (title.includes('auto scaling group') && !title.includes('ec2 instance launch')) {
...
function inferDifficultyAndDuration(task) {
  ...
  if (!difficulty || !['Easy', 'Medium', 'Hard'].includes(difficulty)) {
    if (
      title.includes('placement group') ||
      title.includes('auto recovery') ||
      title.includes('provisioned iops') ||
      title.includes('multi-instance') ||
      title.includes('custom vpc') ||
      title.includes('session manager')
    ) {
      difficulty = 'Hard';
    } else if (
      title.includes('ami') ||
      title.includes('snapshot') ||
      title.includes('iam instance profile') ||
      title.includes('user data') ||
      title.includes('elastic ip') ||
      title.includes('instance store') ||
      title.includes('systems manager') ||
      title.includes('cloudwatch alarm') ||
      consoleCount > 7 ||
      cliCount > 7
    ) {
      difficulty = 'Medium';
...
// EC2 Technical Safety Inspection & Corrections
function sanitizeAndCheckSafety(task) {
    ...
    if (step1.instructions) {
      step1.instructions = step1.instructions.map(ins => {
        ...
        if (text.toLowerCase().includes('root user') || text.toLowerCase().includes('administratoraccess')) {
          corrections.push(`Sanitized Step 1 login instruction to specify IAM user or lab role with EC2 permissions.`);
  ...
  // 2. SSH / RDP Anywhere (0.0.0.0/0) Safety Check
  ...
  const destructivePatterns = [/mkfs/i, /wipefs/i, /rm\s+-rf/i, /umount/i, /delete-volume/i, /delete-snapshot/i, /terminate-instances/i, /deregister-image/i, /disassociate-address/i, /release-address/i];
      ...
      if (s.commands) {
        s.commands.forEach(cmd => {
          ...
          if (isDestructive && !s.warning) {
            s.warning = 'Destructive Command Warning: This command permanently modifies, erases, or terminates AWS resources or local filesystem data.';
  ...
  // 4. Elastic IP Cleanup Warning
  const taskText = JSON.stringify(task).toLowerCase();
  if (taskText.includes('elastic ip') || taskText.includes('eip')) {
    const hasReleaseInCleanup = task.cleanup && task.cleanup.some(c => (c.text || '').toLowerCase().includes('release'));
    if (!hasReleaseInCleanup) {
      task.cleanup.push({
  ...
  // 5. Verify no hardcoded keys or secrets
...
function convertTask(sourceTask, idx) {
  ...
  const rawTitle = cleanHtml(sourceTask.title || sourceTask.sourceHero?.title || `EC2 Task ${sourceId}`);
  const slug = slugify(rawTitle);

  // ID convention: task-saa-ec2-<slug>-00<sourceId>
  const taskId = `task-saa-ec2-${slug}-${String(sourceId).padStart(3, '0')}`;
  ...
  const feature = cleanHtml(sourceTask.feature || 'Elastic Compute Cloud');
  ...
  // Why it matters
  const whyItMatters = cleanHtml(typeof sourceTask.whyThisMatters === 'string' ? sourceTask.whyThisMatters : sourceTask.whyThisMatters?.bodyHtml || `Understanding ${feature} in Amazon EC2 is essential for AWS Solutions Architect Associate exam scenarios and cloud compute design.`);
  ...
  // Cost warning
  const costWarning = cleanHtml(typeof sourceTask.costWarning === 'string' ? sourceTask.costWarning : sourceTask.costWarning?.bodyHtml || 'This lab incurs minimal AWS charges if resources are terminated promptly. Terminate instances, release Elastic IPs, and delete EBS volumes/snapshots during cleanup.');
  ...
  if (sourceTask.verification && Array.isArray(sourceTask.verification)) {
    ...
    verification = [
      { id: 'verify-1', text: `${feature} configuration verified in Amazon EC2.` }
    ];
  }
  ...
  if (sourceTask.cleanup && Array.isArray(sourceTask.cleanup)) {
    ...
    cleanup = [
      { id: 'cleanup-1', text: 'Terminate all test EC2 instances created during this lab.' },
      { id: 'cleanup-2', text: 'Delete any attached EBS volumes and snapshots created during this lab.' }
    ];
  }
  ...
  if (examTips.length === 0) {
    examTips = [
      { id: 'tip-1', text: `SAA-C03: Understand ${feature} configuration and architectural best practices in Amazon EC2.` }
    ];
  }

  // Memory Hook
  const memoryHook = cleanHtml(typeof sourceTask.memoryHook === 'string' ? sourceTask.memoryHook : sourceTask.memoryHook?.bodyHtml || `${feature} in Amazon EC2 provides flexible, scalable compute capability.`);

  // Flashcards reference
  const flashcardSetId = sourceTask.hasFlashcards ? `ec2_task_${sourceId}_flashcards` : null;
...
function runConversion() {
  console.log('--- Starting SAA / EC2 Hands-On Tasks Batch Conversion ---');

  const rawData = fs.readFileSync(SOURCE_EC2_BATCH_PATH, 'utf8');
  const sourceTasks = JSON.parse(rawData);

  console.log(`Loaded ${sourceTasks.length} source records from ${SOURCE_EC2_BATCH_PATH}`);
  ...
  sourceTasks.forEach((sourceTask, idx) => {
    ...
    if (recommendedTopic) {
      issues.push(`Primary objective belongs to topic '${recommendedTopic}' rather than 'topic-ec2'`);
      otherTopicCount++;
    }
  ...
  // 1. Write ec2-converted.json
  const convertedJsonPath = path.join(MIGRATION_DIR, 'ec2-converted.json');
  fs.writeFileSync(convertedJsonPath, JSON.stringify(convertedList, null, 2), 'utf8');
  console.log(`Wrote converted EC2 tasks to: ${convertedJsonPath}`);

  // 2. Write ec2-review-required.json
  const reviewJsonPath = path.join(MIGRATION_DIR, 'ec2-review-required.json');
  fs.writeFileSync(reviewJsonPath, JSON.stringify(reviewRequiredList, null, 2), 'utf8');
  console.log(`Wrote review required EC2 tasks to: ${reviewJsonPath}`);

  // 3. Write ec2-seed.sql
  const sqlSeedPath = path.join(MIGRATION_DIR, 'ec2-seed.sql');
  let sqlContent = `-- SAA / EC2 Converted Tasks Seed SQL (Generated for Review)\n\n`;
  ...
  // 4. Write ec2Tasks.js to src/data/tasks/ec2Tasks.js
  const ec2TasksJsPath = path.join(APP_TASKS_DIR, 'ec2Tasks.js');
  let jsContent = `/**\n * Amazon EC2 Hands-On Tasks & Guided AWS Labs (SAA-C03)\n * Total Converted Tasks: ${convertedList.length}\n */\n\n`;
  jsContent += `export const EC2_TASKS = ${JSON.stringify(convertedList, null, 2)};\n`;
  fs.writeFileSync(ec2TasksJsPath, jsContent, 'utf8');
  console.log(`Wrote ec2Tasks.js to: ${ec2TasksJsPath}`);

  // 5. Write EC2_CONVERSION_REPORT.md
  const reportPath = path.join(MIGRATION_DIR, 'EC2_CONVERSION_REPORT.md');
  let md = `# SAA / EC2 Hands-On Tasks Batch Conversion Report

Generated: ${new Date().toISOString()}

## Executive Summary

* **Total EC2 Source Records**: ${sourceTasks.length}
* **Eligible Records**: ${sourceTasks.length} (all marked \`needs-minor-source-cleanup\`)
* **Converted & Approved**: ${convertedList.length}
* **Integrated into Application**: ${convertedList.length} (in \`src/data/tasks/ec2Tasks.js\`)
...
${reviewRequiredList.length === 0 ? 'No tasks required quarantine. All 27 EC2 tasks passed schema validation and technical safety checks.' : reviewRequiredList.map(r => `- **Task ${r.sourceTaskId} (${r.convertedTask.title})**: ${r.safetyIssues.join(', ')}`).join('\n')}
`;
