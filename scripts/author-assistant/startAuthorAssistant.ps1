...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($env:OPENAI_API_KEY)) {
        ...
        if ([string]::IsNullOrWhiteSpace($plainKey)) {
            throw 'No API key was entered. Nothing was run.'
        }
        $env:OPENAI_API_KEY = $plainKey
        $keyWasProvidedByLauncher = $true
    }

    if ([string]::IsNullOrWhiteSpace($env:AUTHOR_ASSISTANT_MODEL)) {
        $env:AUTHOR_ASSISTANT_MODEL = 'gpt-5.6-terra'
    }

    Push-Location -LiteralPath $projectRoot
    try {
        & node 'scripts\author-assistant\authorAssistant.mjs'
