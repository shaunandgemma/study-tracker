-- Step 009H3A: preserve payment-backed Subscription state during non-payment events.
--
-- Subscription lifecycle events may refresh status, cancellation and period
-- fields, but only a verified invoice.paid activation may replace paid-through
-- or the authoritative paid Invoice reference. Refund manual-review events
-- continue to bypass every Subscription write.
...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION
      '20260912 stopped: the Stripe reconciliation prerequisite is missing.';
  END IF;
END;
$$;
...
COMMENT ON FUNCTION public.reconcile_stripe_exam_entitlement(
  ...
  'Service-role-only Stripe reconciliation. Non-payment lifecycle events preserve paid-through and the authoritative paid Invoice; refund manual-review events preserve every Subscription and learner-access field.';
...
BEGIN
  IF function_oid IS NULL
    ...
    RAISE EXCEPTION
      '20260912 stopped: reconciliation function privileges are incorrect.';
  END IF;
END;
$$;
