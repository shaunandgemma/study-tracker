-- Step 009G5: preserve authoritative Subscription state during refund review.
--
-- Refund webhook events are accepted and recorded, but refund_manual_review with
-- accessAction=no_change must not overwrite status, period, paid-through,
-- latest-invoice or authoritative provider-event fields.
...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION
      '20260911 stopped: the Stripe reconciliation prerequisite is missing.';
  END IF;
END;
$$;
...
COMMENT ON FUNCTION public.reconcile_stripe_exam_entitlement(
  ...
  'Service-role-only Stripe reconciliation. Refund manual-review events are recorded without mutating authoritative Subscription or learner-access state.';
...
BEGIN
  IF function_oid IS NULL
    ...
    RAISE EXCEPTION
      '20260911 stopped: reconciliation function privileges are incorrect.';
  END IF;
END;
$$;
