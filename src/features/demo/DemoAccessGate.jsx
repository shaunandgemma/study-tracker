    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <span className="inline-flex items-center gap-2 rounded-full border border-indigo-700/70 bg-indigo-950/70 px-3 py-1 text-xs font-bold text-indigo-200">
          <ShieldCheck className="h-4 w-4" /> {demoEnabled ? 'Safe demonstration access' : 'Protected account access'}
        </span>
        <h1 className="mt-5 text-3xl font-black tracking-tight text-white sm:text-5xl">
          {demoEnabled ? 'Explore LATT without touching live user data.' : 'Sign in to open your protected learning workspace.'}
        </h1>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
          {demoEnabled
            ...
            : 'Exam access and learner progress are connected to a protected account. Signed-out visitors cannot open an exam workspace or inherit paid access.'}
        </p>
        <div className="mt-7 grid gap-3 sm:grid-cols-3">
          {(demoEnabled ? [
            ...
            ['Private progress', 'Your saved learning progress belongs only to your signed-in account.'],
            ['Verified access', 'Complete exam access is accepted only from a trusted active entitlement.'],
            ['Safe fallback', 'If access cannot be verified, the app safely returns to preview access.']
          ]).map(([title, text]) => (
        ...
        <p className="mt-2 text-xs leading-6 text-slate-400">
          {demoEnabled
            ? 'Registered learners and administrators sign in with their protected account. Visitors use the temporary demo account.'
            : 'Use your protected learner or staff account. No exam workspace is opened while you are signed out.'}
        </p>
        <div className="mt-6 space-y-3">
          {demoEnabled && <button type="button" onClick={onEnterDemo} disabled={enteringDemo} className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">
        ...
        <p className="mt-5 rounded-xl border border-amber-800/50 bg-amber-950/20 p-3 text-[11px] leading-5 text-amber-200">
          {demoEnabled
            ...
            : 'Signed-out access cannot open learner, Author or Approvals workspaces.'}
        </p>
        </aside>
      </div>
