...
import { useAwsConnection } from '../../awsConnection/useAwsConnection.js';
import {
...
export const FollowAlongAwsValidationUnavailable = ({ task }) => (
...
export const getFollowAlongResourceInputConfig = (resourceInput = 'resourceId') => {
...
export const getFollowAlongConnectionMode = (awsConnection) => {
  if (awsConnection?.status === 'connected' && awsConnection.backendVerified) return 'connected';
  if (!awsConnection || awsConnection.status === 'simulation' || !awsConnection.backendVerified) return 'simulation';
  return 'disconnected';
};

export const FollowAlongAwsValidationPanel = ({ task }) => {
  const { awsConnection, openSetup } = useAwsConnection();
  ...
  if (!task) return null;
  if (contracts.length === 0) return <FollowAlongAwsValidationUnavailable task={task} />;

  const reqPermissions = getRequiredPermissionsForTask(task);
  const primaryContract = contracts[0];
  const connectionMode = getFollowAlongConnectionMode(awsConnection);
  const isConnected = connectionMode === 'connected';
  const isSimulation = connectionMode === 'simulation';
  const inputConfig = getFollowAlongResourceInputConfig(primaryContract.resourceInput);
  ...
  const handleRunValidation = async () => {
    ...
    const result = await validateTaskResource(task, awsConnection, resourceInput.trim());
  ...
  return (
    <div className="rounded-3xl bg-slate-900/90 border border-slate-800 p-6 sm:p-8 space-y-6 shadow-xl backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2">
            {isConnected ? (
              <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-800 text-emerald-300 text-xs font-bold flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />Connected (Live STS)
              </span>
            ) : isSimulation ? (
              <span className="px-3 py-1 rounded-full bg-amber-950/80 border border-amber-800 text-amber-300 text-xs font-bold flex items-center gap-1.5">
                <Info className="w-3.5 h-3.5 text-amber-400" />Simulation Mode
              </span>
            ) : (
            ...
            <button type="button" onClick={openSetup}
              className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold underline underline-offset-2">Settings</button>
          </div>
        ) : (
          <button type="button" onClick={openSetup}
            ...
            <Lock className="w-3.5 h-3.5" /><span>Connect AWS Account</span>
          </button>
        )}
      </div>

      {!awsConnection && <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-800/60 text-xs text-amber-200 leading-relaxed flex items-center justify-between gap-3">
        <div className="flex items-center gap-2"><Info className="w-4 h-4 text-amber-400 shrink-0" />
          <span>Connect your AWS account to run live automated validation for this lab task.</span></div>
        <button type="button" onClick={openSetup}
          className="px-3 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-slate-950 font-bold text-xs shrink-0 transition-all">Setup Connection</button>
      </div>}

      <div className="space-y-2 bg-slate-950/60 p-4 rounded-2xl border border-slate-800">
        <label className="block text-xs font-bold text-slate-200 uppercase tracking-wider flex items-center gap-2">
          <Database className="w-4 h-4 text-indigo-400" /><span>{inputConfig.label}</span>
          {inputConfig.required && <span className="text-rose-400">*</span>}
        </label>
        <input type="text" value={resourceInput} onChange={event => setResourceInput(event.target.value)}
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400">
          ...
          {resourceInput.trim().length > 0 && (isInputValid
      ...
      <div className="space-y-2">
        <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Required AWS Read-Only Permissions</label>
        <div className="flex flex-wrap gap-2">
          {reqPermissions.map(permission => <span key={permission}
            className="px-2.5 py-1 rounded-lg bg-slate-950 border border-slate-800 text-[11px] font-mono text-indigo-300">{permission}</span>)}
        </div>
      </div>

      <div className="space-y-3 pt-2">
        <div className="flex flex-wrap items-center gap-3">
          <button type="button" disabled={isValidating || (!canValidate && !isSimulation)} onClick={handleRunValidation}
        ...
        {isInputRequiredAndMissing && isConnected && <p className="text-xs text-rose-400 font-semibold flex items-center gap-1.5">
          <AlertTriangle className="w-3.5 h-3.5" /><span>Enter the {inputConfig.label} above before validating this step.</span>
        </p>}
      </div>

      {validationResult && <div className="space-y-3 pt-4 border-t border-slate-800">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-bold text-slate-200 uppercase tracking-wider">Validation Output &amp; Checks</h4>
          <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${
            validationResult.isSimulation ? 'bg-amber-950 text-amber-300 border-amber-800'
              : validationResult.status === 'live_passed' ? 'bg-emerald-950 text-emerald-300 border-emerald-800'
                : 'bg-rose-950 text-rose-300 border-rose-800'
          }`}>{validationResult.isSimulation ? 'Simulation Checked'
            : validationResult.status === 'live_passed' ? 'Live AWS Verified' : 'Live AWS Failed'}</span>
        </div>
        <p className="text-xs text-slate-300">{validationResult.message}</p>
        <div className="space-y-2">
          {validationResult.results.map(result => <div key={result.id}
            className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex items-start gap-3 text-xs">
            {result.passed ? <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
              : <XCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" />}
            <div className="space-y-1"><div className="font-semibold text-slate-200">{result.text}</div>
              <div className="text-[11px] text-slate-400 font-mono">{result.message}</div></div>
          </div>)}
        </div>
      </div>}
    </div>
