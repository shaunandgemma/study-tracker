...
import { AlertTriangle, CheckCircle2, Copy, Download, FileLock2, ShieldAlert } from 'lucide-react';
import { AuthorCandidateReadinessPreview } from './AuthorCandidateReadinessPreview.jsx';
...
export function AuthorApprovalStage({ draft, userId, authorEmail, storageMode, onPreviewCandidateReadiness, onStoreReleaseCandidate, planningValidation, contentValidation, reviewValidation, setMessage }) {
  ...
  const requiresStep94Readiness = isShared && isStep94VerifiedSqsDraft(draft);
  ...
  const copyCandidateId = async () => {
    try {
      await globalThis.navigator?.clipboard?.writeText(candidate.candidateId);
      setMessage('Candidate Approval Key copied. Give it only to the separate trusted Approver.');
    } catch {
      setMessage('The browser could not copy the key. Select the full Candidate Approval Key and copy it manually.');
    }
  };
  ...
  return <div className="space-y-5">
    ...
    <AuthorCandidateReadinessPreview draft={draft} authorEmail={authorEmail} storageMode={storageMode} planningValidation={planningValidation} contentValidation={contentValidation} reviewValidation={reviewValidation} onPreview={onPreviewCandidateReadiness} onReadinessChange={setReadinessConfirmed} setMessage={setMessage} />
    <section className="rounded-2xl border border-slate-800 bg-slate-900/90 p-5 space-y-4"><div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs"><div><span className="text-slate-500 block">Review state</span><strong className={draft.review?.reviewStatus === 'ready_for_approval' ? 'text-emerald-300' : 'text-amber-300'}>{draft.review?.reviewStatus || 'in_review'}</strong></div><div><span className="text-slate-500 block">Approval decision</span><strong className="text-white">Pending</strong></div><div><span className="text-slate-500 block">Publication</span><strong className="text-white">Not published</strong></div></div><button type="button" disabled={working || (requiresStep94Readiness && !readinessConfirmed)} onClick={prepare} className="px-4 py-2.5 rounded-xl bg-cyan-600 disabled:opacity-50 text-xs font-bold text-white inline-flex items-center gap-2"><FileLock2 className="w-4 h-4" /> {working ? 'Checking...' : requiresStep94Readiness && !readinessConfirmed ? 'Preview Readiness First' : candidate ? isShared ? 'Prepare New Candidate' : 'Prepare New Local Package' : isShared ? 'Prepare Shared Release Candidate' : 'Prepare Local Package'}</button></section>
    {candidate && <section className="rounded-2xl border border-cyan-800 bg-cyan-950/20 p-5 space-y-4"><div className="flex items-start gap-3"><FileLock2 className="w-6 h-6 text-cyan-300 shrink-0" /><div className="min-w-0 flex-1"><strong className="text-sm text-white">{isShared ? 'Immutable shared candidate prepared' : 'Immutable local package prepared'}</strong><span className="block text-[11px] font-bold uppercase tracking-wider text-cyan-300 mt-3">{isShared ? 'Candidate Approval Key' : 'Local Package ID'}</span><code className="block mt-1 rounded-xl border border-cyan-800 bg-slate-950 p-3 text-xs text-cyan-100 break-all select-all">{candidate.candidateId}</code>{isShared && <button type="button" onClick={() => void copyCandidateId()} className="mt-2 px-4 py-2 rounded-xl bg-cyan-700 text-xs font-bold text-white inline-flex items-center gap-2"><Copy className="w-4 h-4" /> Copy Candidate Approval Key</button>}</div></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs"><div><span className="text-slate-500 block">Source revision</span><strong className="text-white">{candidate.sourceRevision}</strong></div><div><span className="text-slate-500 block">Status</span><strong className="text-amber-300">{isShared ? 'Awaiting trusted approval' : 'Local package only — approval unavailable'}</strong></div><div className="sm:col-span-2"><span className="text-slate-500 block">SHA-256 content fingerprint</span><code className="text-[11px] text-cyan-200 break-all">{candidate.contentHash}</code></div></div>{isShared && <div className="rounded-xl border border-slate-700 bg-slate-950/60 p-3 text-xs text-slate-300"><strong className="text-white">Next:</strong> A different Approver opens <code className="text-cyan-200">http://127.0.0.1:5173/#author/approvals</code> and pastes this key into the confirmation box.</div>}{integrity && <div className={`rounded-xl border p-3 text-xs flex items-start gap-2 ${integrity.valid && integrity.matches ? 'border-emerald-800 bg-emerald-950/20 text-emerald-200' : 'border-rose-800 bg-rose-950/20 text-rose-200'}`}>{integrity.valid && integrity.matches ? <CheckCircle2 className="w-4 h-4 shrink-0" /> : <AlertTriangle className="w-4 h-4 shrink-0" />}<span>{integrity.valid && integrity.matches ? 'Candidate content and current draft revision match.' : 'Candidate integrity or revision does not match. Do not approve this package.'}</span></div>}<div className="flex flex-wrap gap-2"><button type="button" disabled={working} onClick={check} className="px-4 py-2 rounded-xl bg-slate-700 text-xs font-bold text-white">Check Integrity</button><button type="button" onClick={() => { if (!downloadCandidate(candidate)) setMessage('The browser could not download the candidate.'); }} className="px-4 py-2 rounded-xl bg-slate-700 text-xs font-bold text-white inline-flex items-center gap-2"><Download className="w-4 h-4" /> Download JSON Package</button></div></section>}
