...
function ProgrammeStage({ draft, change }) {
  ...
  return <div className="space-y-5"><div><h2 className="text-xl font-extrabold text-white">AWS service and learner outcome</h2><p className="text-sm text-slate-400 mt-1">Describe one connected project in plain language.</p></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <Field label="Estimated total minutes"><input type="number" min="1" value={p.estimatedMinutes ?? ''} onChange={event => change('estimatedMinutes', event.target.value === '' ? null : Number(event.target.value))} placeholder="240" className={inputClass} /></Field>
...
export const AuthorDraftEditor = ({ initialDraft, userId, storageMode = 'private_local_browser', onSaveDraft, onStoreReleaseCandidate, onSavedAndExit, onCancel }) => {
