...
export function SourcesStage({ draft, apply, setMessage }) {
  ...
  const add = event => {
    ...
    apply(result.draft); setForm({ title: '', url: '', purpose: '' }); setMessage('Official source added. Link it to the tasks it supports.');
  };
  return <div className="space-y-5"><div><h2 className="text-xl font-extrabold text-white">Official documentation sources</h2><p className="text-sm text-slate-400 mt-1">Record the official pages used to write and check each task. Official HTTPS AWS and HashiCorp Terraform links are accepted.</p></div><form onSubmit={add} className={cardClass}><Field label="Official document title"><input value={form.title} onChange={event => setForm(value => ({ ...value, title: event.target.value }))} placeholder="Create a VPC" className={inputClass} /></Field><Field label="Official documentation address"><input type="url" value={form.url} onChange={event => setForm(value => ({ ...value, url: event.target.value }))} placeholder="https://docs.aws.amazon.com/..." className={inputClass} /></Field><Field label="Why is this source being used?"><textarea value={form.purpose} onChange={event => setForm(value => ({ ...value, purpose: event.target.value }))} rows={2} className={inputClass} /></Field><RunButton>Add Official Source</RunButton></form><div className="space-y-4">{(draft.sources || []).map(source => <article key={source.id} className={cardClass}><div className="flex items-start justify-between gap-3"><div><strong className="text-sm text-white">{source.title || 'Untitled official source'}</strong><span className="block text-[11px] text-slate-500 mt-1">Stable ID: {source.id}</span></div><RemoveButton label="Remove source" onClick={() => run(removeAuthorSource(draft, source.id))} /></div><Field label="Document title"><input value={source.title} onChange={event => run(updateAuthorSource(draft, source.id, { title: event.target.value }))} className={inputClass} /></Field><Field label="Official documentation address"><input value={source.url} onChange={event => run(updateAuthorSource(draft, source.id, { url: event.target.value }))} className={inputClass} /></Field><Field label="Reason for using this source"><textarea value={source.purpose} onChange={event => run(updateAuthorSource(draft, source.id, { purpose: event.target.value }))} rows={2} className={inputClass} /></Field><fieldset className="rounded-xl border border-slate-800 p-3"><legend className="px-1 text-xs font-semibold text-slate-300">Tasks supported by this source</legend><div className="space-y-2 mt-2">{draft.tasks.map(task => <label key={task.id} className="flex items-center gap-2 text-xs text-slate-300"><input type="checkbox" checked={(source.taskIds || []).includes(task.id)} onChange={event => run(setAuthorSourceTaskLink(draft, source.id, task.id, event.target.checked))} /> {task.title}</label>)}</div></fieldset></article>)}</div>{!(draft.sources || []).length && <p className="text-sm text-slate-500">No official sources yet.</p>}</div>;
}
