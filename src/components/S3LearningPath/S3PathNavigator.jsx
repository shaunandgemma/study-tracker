...
export const S3PathNavigator = ({ activeTaskId, completedTaskIds = [], onSelectTask }) => {
    ...
    <div className="p-4 space-y-6">
      <div className="border-b border-slate-800 pb-3">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
          S3 Learning Path Outline
        </h3>
        <p className="text-[11px] text-slate-500 mt-0.5">33 Tasks across 7 Phases</p>
      </div>
        ...
        {S3_LEARNING_PATH_PHASES.map(phase => {
          ...
          return (
            <div key={phase.id} className="space-y-1.5">
              <div className="text-[11px] font-bold text-amber-400/90 uppercase tracking-wider px-2">
                Phase {phase.id}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-2.5 py-2 rounded-md text-xs transition-all flex items-start gap-2.5 ${
                        isActive
                          ? 'bg-amber-500/15 border border-amber-500/40 text-amber-300 font-medium shadow-sm'
                          : isDone
