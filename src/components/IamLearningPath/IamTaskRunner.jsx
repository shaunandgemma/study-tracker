    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-purple-400">
              {task.id}
            </span>
            {task.isOptional && (
        ...
        {/* Mode Selector & Complete Button */}
        <div className="flex items-center gap-3">
          <div className="flex items-center bg-slate-950 p-1 rounded-lg border border-slate-800">
            <button
              onClick={() => onModeChange('console')}
              className={`flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium transition-all ${
                preferredMode === 'console' ? 'bg-purple-600 text-white font-bold' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium transition-all ${
                preferredMode === 'cli' ? 'bg-purple-600 text-white font-bold' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium transition-all ${
                preferredMode === 'both' ? 'bg-purple-600 text-white font-bold' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${
              isCompleted
                ...
                : 'bg-slate-800 border border-slate-700 text-slate-200 hover:border-purple-500/50'
            }`}
          >
      ...
      {task.whyItMatters && (
        <div className="bg-slate-950/60 border border-slate-800/80 rounded-lg p-4 text-xs text-slate-300 leading-relaxed">
          <strong className="text-purple-400 block mb-1">Why This Matters:</strong>
          {task.whyItMatters}
        </div>
      )}
      ...
      {(preferredMode === 'console' || preferredMode === 'both') && task.consoleSteps && task.consoleSteps.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2">
            <Layout className="w-4 h-4 text-purple-400" /> AWS Management Console Instructions
          </h3>
                  ...
                  {step.instructions && step.instructions.map((ins, iIdx) => (
                    <label key={ins.id || iIdx} className="flex items-start gap-2.5 text-xs text-slate-300 cursor-pointer hover:text-slate-100">
                      <input
                        ...
                        className="mt-0.5 rounded border-slate-700 bg-slate-900 text-purple-500 focus:ring-purple-500/20"
                      />
      ...
      {(preferredMode === 'cli' || preferredMode === 'both') && task.cliSteps && task.cliSteps.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2">
            <Terminal className="w-4 h-4 text-purple-400" /> AWS CLI Command Instructions
          </h3>
