...
export const ExamSetup = ({ onStartExam, presetConfig, onViewAttempt }) => {
  ...
  const [selectedDomainId, setSelectedDomainId] = useState(presetConfig?.domainId || allTopicsList[0]?.id || '');
  ...
  const demoTopicIds = useMemo(() => new Set(fullExamQuestions.flatMap(question => [
    question.topicId || question.domainId,
  ...
  useEffect(() => {
    if (topicsList.length && !topicsList.some(topic => topic.id === selectedDomainId)) {
      setSelectedDomainId(topicsList[0].id);
    }
  }, [topicsList, selectedDomainId]);
